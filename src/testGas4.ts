...
async function test() {
    ...
    const unprocessed = formattedTuti.filter(e => !e.ketLuan || e.ketLuan.trim() === '');
    console.log("Empty or whitespace TU/TI summary:");
    formattedTuti.forEach(u => {
        if (!u.thongSoTU?.trim() || !u.thongSoTI?.trim()) {
            console.log(`- Tram: ${u.id}, TU: '${u.thongSoTU}', TI: '${u.thongSoTI}'`);
        }
    });
}
test();
