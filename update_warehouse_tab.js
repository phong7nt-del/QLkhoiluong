...
import { Plus, Trash2, X, Settings, Package, Info, Upload, Save } from 'lucide-react';
...
export default function WarehouseTab() {
  ...
  const saveKhoToSheetAndLocal = async (newZones: Zone[], w: number, h: number) => {
    setZones(newZones);
    setWarehouseWidth(w);
    ...
    const khoRows = [
       { id: 'WAREHOUSE_SIZE', type: 'SIZE', code: 'MAIN', x: 0, y: 0, width: w, height: h, categories: '' },
       ...newZones.map(z => ({
           id: z.id,
           type: 'ZONE',
           code: z.code,
    ...
    // In background, sync to DataStore
    DataStore.syncKhoToSheet(khoRows);
  };
  ...
  const addZone = () => {
    ...
    saveKhoToSheetAndLocal([...zones, newZone], warehouseWidth, warehouseHeight);
  };

  const deleteZone = (id: string) => {
    saveKhoToSheetAndLocal(zones.filter(z => z.id !== id), warehouseWidth, warehouseHeight);
  ...
  const updateZone = (id: string, updates: Partial<Zone>) => {
    saveKhoToSheetAndLocal(zones.map(z => z.id === id ? { ...z, ...updates } : z), warehouseWidth, warehouseHeight);
  };
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex gap-4 overflow-hidden p-4">
      <div className="flex-1 flex flex-col bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-auto bg-slate-100 p-4 flex items-start justify-start relative">
           <div 
           ...
           >
              {zones.map(zone => (
                 <div
                   ...
                   className={\`absolute flex flex-col justify-center items-center text-center p-2 rounded-md transition-shadow
                     \${isEditMode ? 'cursor-move border-2 border-dashed border-blue-400 bg-blue-50/50 hover:bg-blue-100/50' : 'cursor-pointer border border-slate-400 bg-white hover:bg-slate-50 shadow-sm'}
                     \${selectedZone?.id === zone.id ? 'ring-2 ring-blue-500 border-blue-500 bg-blue-50' : ''}
                   \`}
                   style={{ 
                 ...
                 >
                    <span className="font-bold text-slate-800 block text-sm drop-shadow-sm">{zone.code}</span>
                    <span className="text-[10px] text-slate-500 block mt-1">{zone.categories.length} chủng loại</span>
                 </div>
              ))}
           </div>
        </div>
      ...
      {(selectedZone || isEditMode) && (
        <div className="w-[420px] bg-white rounded-lg shadow-sm border border-slate-200 flex flex-col overflow-hidden shrink-0">
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={e => {
                            ...
                            saveKhoToSheetAndLocal(zones, val, warehouseHeight);
                          }}
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={e => {
                            ...
                            saveKhoToSheetAndLocal(zones, warehouseWidth, val);
                          }}
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                        />
                      </div>
                  ...
                  <button 
                    onClick={() => {
                       if (window.confirm("Bạn có chắc muốn reset toàn bộ sơ đồ kho?")) {
                          saveKhoToSheetAndLocal([], warehouseWidth, warehouseHeight);
                       }
                    }}
                    className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-md text-sm font-bold border border-red-100 hover:bg-red-100"
                  >
