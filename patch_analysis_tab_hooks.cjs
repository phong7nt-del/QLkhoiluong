...
const t1 = `  const [selectedMonth, setSelectedMonth] = useState<string>(format(new Date(), 'yyyy-MM'));`;
const r1 = `  const [selectedMonth, setSelectedMonth] = useState<string>(format(new Date(), 'yyyy-MM'));
...
if (code.includes(t1) && code.includes(t2) && code.includes(t3)) {
    ...
    console.log("Success: added config hooks to AnalysisTab");
} else {
